...
class _ProfileScreenState
  ...
  String _activeType =
      'indoor'; // Varsayılan olarak indoor seçili
  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      ref.refresh(
          userRanksProvider); // Kullanıcı sıralamasını yükle
      ref.refresh(
          userStreakProvider); // Kullanıcı streak bilgisini yükle
    });
  }
    ...
    try {
      ...
      for (var activity in activities) {
        ...
        final DateTime activityDate =
            activity.startTime;
        final int weekday = activityDate.weekday -
            1; // 0-6 aralığına dönüştür
      ...
      debugPrint(
          'Aktivite verisi işlenirken hata: $e');
  ...
  Map<String, double> _calculateSummary(
    ...
    for (var activity in activities) {
      ...
      if (activity.avarageSpeed != null) {
        totalSpeed +=
            activity.avarageSpeed!.toDouble();
        speedCount++;
      }
    }

    double averageSpeed = speedCount > 0
        ? totalSpeed / speedCount
        : 0.0;

    // Adımları km'ye çevir
    double stepsInKm =
        stepsToKilometers(totalSteps.toInt());
  ...
  double _calculateMaxY(List<FlSpot> spots) {
    if (spots.isEmpty)
      return _activeType == 'indoor' ? 1000 : 5;

    double maxValue = spots
        .map((e) => e.y)
        .reduce((a, b) => a > b ? a : b);
  ...
  double _calculateYAxisInterval(
      List<FlSpot> spots) {
    if (spots.isEmpty)
      return _activeType == 'indoor' ? 250 : 1;
  ...
  final recentRacesProvider =
    ...
    try {
      ...
      debugPrint(
          'Yarış sonuçları provider hatası: $e');
      return [];
    }
  });

  // Asset paths (adjust if necessary)
  static const String _treadmillImage =
      'assets/icons/indoor.png';
  static const String _outdoorImage =
      'assets/icons/outdoor.png';
  static const String _flameIcon =
      'assets/icons/alev.png';
  static const String _locationIcon =
      'assets/icons/location.png';
  static const String _stepsIcon =
    ...
    return Column(
      ...
      children: [
        Image.asset(
          assetPath,
          width:
              32, // Adjusted size based on image
          height:
              32, // Adjusted size based on image
          errorBuilder:
  ...
  Widget build(BuildContext context) {
    ...
    final userRanksAsync =
        ref.watch(userRanksProvider);
    ...
    return Scaffold(
      backgroundColor: const Color.fromARGB(
          ...
          0,
          0,
          0), // Dark background color from image
      body: SafeArea(
                      ...
                      children: [
                        ...
                        Positioned(
                          ...
                          child: IconButton(
                            icon: const Icon(
                                Icons
                                    .discount_outlined, // Using outlined icon
                                color: Color(
                        ...
                        Positioned(
                          ...
                          child: IconButton(
                            icon: const Icon(
                                Icons
                                    .settings_outlined, // Using outlined icon
                                color: Color(
                                    0xFF93C53E)), // Green color from image
                            onPressed: () {
                              Navigator.push(
                                        ...
                                        LineChartData(
                                              ...
                                              getTooltipItems:
                                                ...
                                                return touchedSpots.map((spot) {
                                                  return LineTooltipItem(
                                                    ...
                                                    const TextStyle(
                                                      color: Colors.black, // Black text on green tooltip
                                          ...
                                          gridData:
                                            ...
                                            getDrawingHorizontalLine:
                                                (value) {
                                              return FlLine(
                                                color: Colors.white.withOpacity(0.1), // Dimmer grid lines
                                                strokeWidth: 1,
                                              );
                                            },
                                            ...
                                            leftTitles:
                                              ...
                                              sideTitles:
                                                ...
                                                getTitlesWidget: (value, meta) {
                                                  ...
                                                  final displayValue = _activeType == 'indoor' ? value.toInt().toString() : value.toStringAsFixed(value.truncateToDouble() == value ? 0 : 1); // Show .0 if whole number for km
                                          ...
                                          lineBarsData: [
                                            LineChartBarData(
                                              ...
                                              preventCurveOverShooting:
                                                  true,
                                              color:
                                                  const Color(0xFF93C53E), // Green line color
                                              barWidth:
                                                  4, // Thicker line
                                              isStrokeCapRound:
                                                  true,
                                              dotData: FlDotData(
                                                  ...
                                                  getDotPainter: (spot, percent, barData, index) {
                                                    ...
                                                    final isMax = spot.y == spots.map((e) => e.y).reduce(math.max);
                                                    return FlDotCirclePainter(
                                                      radius: isMax ? 5 : 0, // Show only max dot
                                                      color: const Color(0xFF93C53E),
                                                      strokeWidth: 2,
                                                      strokeColor: const Color(0xFF1E1E1E), // Match background
                                                    );
                                                  },
                                                  checkToShowDot: (spot, barData) {
                                                    return spot.y > 0; // Don't show dots for 0 values visually (handled by radius above)
                                                  }),
                                              belowBarData:
                                                  BarAreaData(
                                                // Gradient below line
                                                ...
                                                gradient: LinearGradient(
                                                  colors: [
                                                    const Color(0xFF93C53E).withOpacity(0.3),
                                                    const Color(0xFF93C53E).withOpacity(0.0),
                                                  ],
                        ...
                        Row(
                          ...
                          children: [
                            const Text(
                              ...
                              style: TextStyle(
                                fontSize:
                                    18, // Match performance title size
                                fontWeight:
                                    FontWeight
                                        .bold,
                                color:
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    _buildNewRaceMetricItem(
                                                      ...
                                                      valueText: '$distanceStr km',
                                                    ),
                                                    const SizedBox(width: 20),
                                                    _buildNewRaceMetricItem(
                                                      assetPath: _stepsIcon,
                                                      valueText: stepsStr, // As per image, no "steps" unit
                                                    ),
                                                  ],
                                                ),
                                              ],
          ...
          error: (error, stackTrace) {
            ...
            return Center(
              child: NetworkErrorWidget(
                ...
                message:
                    'Profil bilgileri alınamadı, lütfen tekrar deneyin.',
                onRetry: () {
                  ref.invalidate(
  ...
  Widget _buildFilterButton(
      String typeKey, bool isSelected) {
    String buttonText;
    switch (typeKey.toLowerCase()) {
      ...
      case 'record':
        buttonText =
            'Kayıt'; // Assuming 'Record' maps to 'Kayıt'
        break;
      default:
        buttonText = typeKey;
    }
          ...
          data: (streakCount) {
            return _buildStatIcon(
              ...
              iconColor: const Color(
                  0xFFFFA000), // Orange/Yellow color for fire
              backgroundColor: const Color(
                      0xFFFFA000)
                  .withOpacity(
                      0.15), // Match icon color opacity
            );
          },
          loading: () => Expanded(
            ...
            child: Column(
              ...
              children: [
                const SizedBox(
                    width: 36,
                    height:
                        36, // Match icon container size
                    child: Padding(
                      padding: EdgeInsets.all(
                          8.0), // Padding inside circle
                      child:
                          CircularProgressIndicator(
                              color:
                                  Colors.white54,
        ...
        children: [
            ...
            decoration: BoxDecoration(
              color: backgroundColor ??
                  Colors.white.withOpacity(
                      0.1), // Use provided bg or default
          ...
          Text(
            ...
            style: const TextStyle(
              color: Colors.white,
              fontSize:
                  18, // Slightly smaller value text
              fontWeight: FontWeight.bold,
            ),
          ),
          const SizedBox(height: 4),
  ...
  Widget _buildStatsContainer(
      UserRanksModel? userRanks) {
    final userCoinsAsync =
        ref.watch(userDataProvider).value?.coins;
    return Container(
      ...
      child: IntrinsicHeight(
        ...
        child: Row(
          ...
          children: [
            _buildStatIcon(
              icon: Icons
                  .home_outlined, // Use outlined icon
              value: userRanks?.indoorRank
                      .toString() ??
                  "-",
              label: 'İç', // Label from image
              iconColor: const Color(
                  0xFF93C53E), // Green icon
              backgroundColor: const Color(
                      0xFF93C53E)
                  .withOpacity(
                      0.15), // Green background
            ),
            _buildStatIcon(
              icon: Icons
                  .terrain_outlined, // Use outlined icon (closer to mountain)
              value: userRanks?.outdoorRank
                      .toString() ??
                  "-",
            ...
            _buildStatIcon(
              ...
              value: userCoinsAsync
                      ?.toStringAsFixed(2) ??
                  '0.00', // Format to 2 decimal places
              label:
                  'Movliq Coin', // Label from image
              iconColor: Colors
                  .amber, // Gold color for coin
              backgroundColor: const Color(
                      0xFFFFD700)
                  .withOpacity(
                      0.15), // Gold background
            ),
          ],
        ),
      ),
  ...
  Widget _buildSummaryStats(
    ...
    final totalSteps =
        summary['totalSteps']?.toInt() ?? 0;
    final estimatedKm =
        summary['stepsInKm'] ?? 0.0;
    final totalDistanceKm =
        summary['totalDistance'] ??
            0.0; // Get total distance
    final totalCalories =
        summary['totalCalories']?.toInt() ?? 0;
    ...
    if (_activeType == 'indoor') {
      ...
      middleIcon =
          Icons.timeline; // Or Icons.straighten
    } else {
      // Outdoor or Record
      middleValue =
      ...
      middleIcon = Icons
          .route_outlined; // Route icon for outdoor distance
    }
      ...
      child: Row(
        ...
        children: [
          ...
          _buildSummaryStatItem(
            ...
            value:
                middleValue, // Use dynamic value
            label:
                middleLabel, // Use dynamic label
            iconColor: const Color(0xFF93C53E),
          ),
          SizedBox(
            width: 30,
          ...
          _buildStatIcon(
            ...
            label: 'Toplam Kalori',
            iconColor: const Color(
                0xFFFFA000), // Orange for calories
          ),
        ],
      ),
    );
...
class ProfilePictureWidget
  ...
  @override
  State<ProfilePictureWidget> createState() =>
      _ProfilePictureWidgetState();
}

class _ProfilePictureWidgetState
    extends State<ProfilePictureWidget> {
  ...
  Future<void> _selectAndUploadProfileImage(
    ...
    try {
      ...
      final response = await _uploadProfileImage(
          image.path, tokenJson);
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _imageKey =
              UniqueKey(); // Ensure image rebuilds with new data
          _localImageFile =
              null; // Clear local file reference
        });
      } else {
        _showErrorMessage(context,
            'Profil fotoğrafı yüklenirken bir hata oluştu');
        _localImageFile =
            null; // Hata durumunda yerel dosyayı temizle
      }
    } catch (e) {
      ...
      builder: (context, ref, child) {
        ...
        final UserDataModel currentUserData =
          ...
          data: (userDataFromProvider) =>
              ...
              widget
                  .userData, // Use provider data if available
          orElse: () => widget.userData,
        );

        final profileUrl =
            currentUserData.profilePictureUrl;
        final gender = currentUserData
            .gender; // Get gender from currentUserData

        // Define default asset paths
        const String defaultManPhoto =
            'assets/images/defaultmanphoto.png';
        ...
        final ImageProvider imageProvider =
            _localImageFile != null
                ? FileImage(
                    _localImageFile!) // Show local file if selected
                : (profileUrl != null &&
                        profileUrl.isNotEmpty
                    ? NetworkImage(
                        profileUrl) // Use NetworkImage directly
                    : AssetImage(
                        selectedDefaultImageAsset)); // Use gender-specific fallback asset

        const double imageSize =
            100; // Increased size
        const double cameraIconSize =
            30; // Size of the camera icon circle
        const double cameraIconPositionOffset =
            -5; // Offset for camera icon
        ...
        return GestureDetector(
          onTap: _isUploading
              ...
              : () => _selectAndUploadProfileImage(
                  context,
                  ref), // Disable tap during upload
          child: Stack(
            clipBehavior: Clip
                .none, // Allow camera icon to overflow
            ...
            children: [
              ...
              if (!_isUploading) // Only show camera icon when not uploading
                Positioned(
                  ...
                  bottom:
                      cameraIconPositionOffset,
                  child: Container(
                    width: cameraIconSize,
                    height: cameraIconSize,
                    decoration: BoxDecoration(
                      color: const Color(
                          0xFFC4FF62), // Bright green from image
                      shape: BoxShape.circle,
                      border: Border.all(
                        color: const Color(
                    ...
                    child: const Icon(
                      Icons
                          .camera_alt_rounded, // Rounded camera icon
                      color: Colors.black,
                      size:
                          16, // Adjust icon size within circle
                    ),
                  ),
                ),
            ],
